...
void sum(gsl_vector* lambda,gsl_vector* eigen,gsl_vector* d, gsl_vector* u, int p){
	int n = lambda->size;
//	int j = u->size;
...
void eigen(gsl_matrix* A, gsl_vector* x, gsl_vector* d, gsl_vector* u, int p){
	int n = A->size1;
...
//void sum(gsl_vector* lambda,gsl_vector* eigen,gsl_vector* d, gsl_vector* u, int p){
	...
	gsl_vector_free(u);
	p = 2;	dpi = 0;
	printf("#index 0: dimension, time\n");

	
	
	/*	for(int i=5;i<150;i+=20){
		gsl_matrix* A1 = gsl_matrix_calloc(i,i);
//		show_matrix(A1);
//		realloc(A1,12);
//		show_matrix(A1);
		gsl_vector* u = gsl_vector_calloc(i);
		gsl_vector* d = gsl_vector_calloc(i);
		gsl_vector* result = gsl_vector_calloc(i);
		for(int k=0;k<i;k++){
			dpi = rnd;
			gsl_matrix_set(A1,k,k,dpi);
			fprintf(stderr,"FUCK!%i\n",k);
			gsl_vector_set(u,k,rnd);
			gsl_vector_set(d,k,dpi);
			if(i==p){
				gsl_vector_set(u,p,0);}
		}
		fprintf(stderr,"So far so FUCKING GOD!\n");
		start = clock();
		eigen(A1,u,result,d,p);
		end = clock();
		cpu = ((double) (end - start)) / CLOCKS_PER_SEC;
		printf("%i %.10g\n",i,cpu);
		gsl_vector_free(d);
