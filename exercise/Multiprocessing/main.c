...
void* my_pi(void* args){
	...
	for(int i=0;i<N;i++){
		...
		double norm = sqrt(xpoint*xpoint + ypoint*ypoint);
		if(norm<=1)(*data).Ninc++;
	}
return NULL;
}
...
#pragma omp parallel sections
	...
	printf("My monte carlo simulation of pi gives %.10g.\nUsing OpemMP, I get %.10g\nFor reference, the value of pi from math.h is %.10g \n",pi,op_pi,M_PI);
return 0;	
}
