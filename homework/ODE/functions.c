...
/*
void rkstep12(void f( double t, gsl_vector* y, gsl_vector* dydt), double t, double* yt,double h, gsl_vector* yh, gsl_vector* err){
	...
	f(t+h/2,yt,k12);
	for (int i=0;i<n;i++){
		gsl_vector_set(yh,i,yt[i] + k12[i]*h);
	}
	for(int i =0;i<n;i++){
		gsl_vector_set(err,i,k0[i]-k12[i]*h/2);
	}
}
*/
