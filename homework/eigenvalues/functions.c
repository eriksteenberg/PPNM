...
void timesJ(gsl_matrix* A, int p, int q, double theta){
	...
	for(int i=0;i<A->size1;i++){
		double new_aiq = -s*gsl_matrix_get(A,i,q)+c*gsl_matrix_get(A,i,p);
		double new_aip = c*gsl_matrix_get(A,i,q)+s*gsl_matrix_get(A,i,p);
...
//A little test to see how a single jacobi rotation effects the system
...
void j(gsl_matrix* A,double q,double p){
	...
	double x = atan2(2*gsl_matrix_get(A,p,q),gsl_matrix_get(A,q,q)-gsl_matrix_get(A,p,p));
	gsl_matrix_set(J,q,q,cos(x));
	gsl_matrix_set(J,p,p,cos(x));
	gsl_matrix_set(J,q,p,-sin(x));
	gsl_matrix_set(J,p,q,sin(x));
	gsl_blas_dgemm(CblasNoTrans,CblasNoTrans, 1.0,A,J,0.0,I);
	gsl_blas_dgemm(CblasTrans,CblasNoTrans, 1.0,J,I,0.0,A);
	gsl_matrix_free(J);
	gsl_matrix_free(I);
}
